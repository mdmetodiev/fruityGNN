...
class Molecule:
    ...
    def __post_init__(self):
        ...
        mol_weight = Descriptors.MolWt(self._mol)
        logp = Descriptors.MolLogP(self._mol)
        self.global_context_vector = (mol_weight, logp)
    ...
    def _create_edges_and_adjacency_list(self):
        ...
        for bond in self._mol.GetBonds():
            ...
            bond_type = bond_type_mapping.get(bond.GetBondType(), 0)  # Encode bond type
            edges.append(bond_type)
            adjacency_list.append((j, i))  # Store adjacency as (connected node indices)
    ...
    def get_atomic_positions(self) :
        ...
        #conformer = self._mol.GetConformer(0)  # Get 2D conformer
...
m1 = Molecule("CC(C)CC(=O)OCCCC")
m2 = Molecule("CCCC(C)C(=O)OCC")
